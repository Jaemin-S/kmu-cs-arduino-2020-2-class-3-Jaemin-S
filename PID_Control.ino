...
////////////////////
  ...
  if(event_dist) {
    ...
    if(iterm < - _ITERM_MAX) iterm = - _ITERM_MAX;
    if (error_curr > -2 && error_curr <= -1) dterm = (0.9 + -0.1 * error_curr) * _KD * (error_curr - error_prev);
    else if (error_curr <= -2) dterm = 1.1 * _KD * (error_curr - error_prev);
    else dterm = 1.4 * _KD * (error_curr - error_prev);
