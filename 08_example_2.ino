...
//  }
  if(100 <= dist_raw && dist_raw <= 200) {
    analogWrite(PIN_LED, int(255-2.55*(dist_raw-100)));
  }
  if(200 < dist_raw && dist_raw <= 300) {
    analogWrite(PIN_LED, int(2.55*(dist_raw-200)));
  }
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if (reading != 0) {save_reading = reading;}
  else {reading = save_reading;}
  return reading;
